...
export class TicketsService {
  ...
  async create(userId: number, data: { flightId: number; returnFlightId?: number; classId: number; seatId: number; passengers: any[] }) {
    ...
    // per-flight class pricing override (preferred)
    ...
    let perSeat: number;
    if (override?.price && isFinite(Number(override.price))) {
      perSeat = Number(override.price);
    } else {
      // fallback to basePrice * class multiplier
      const base = Number((flight as any).basePrice || 0) || 0;
      perSeat = Number((base * (mult || 1)).toFixed(2));
    }
  ...
  async myTickets(userId: number) {
    // Booking has two relations to Flight (flight and returnFlight); specify aliases explicitly
