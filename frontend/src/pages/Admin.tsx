          ...
          <div className="row" style={{gap:8}}>
            {classList.map((c:any)=> (
              <div key={c.id} className="field" style={{minWidth:240}}>
                <label>{c.name} ({c.code}) — Narx va limit</label>
                <div className="row" style={{gap:6, alignItems:'center'}}>
                  ...
                  <input placeholder="Seat limit (ixtiyoriy)" type="number" value={classCfg[c.id]?.seatLimit||''} onChange={e=>setClassCfg(p=>({ ...p, [c.id]: { ...(p[c.id]||{}), seatLimit: Number(e.target.value)||undefined } }))} />
                </div>
              </div>
            ))}
          </div>
